# Pythia

Pythia is a myth-tech story world where Fieldlight architecture becomes character, conflict, interface, and consequence.

It is part oracle system, part hacker-house arena, part intelligence market, part astral underworld. It translates questions of signal, extraction, memory, consent, erotic power, governance, and authorship into a world people can enter.

## Core Frame

Pythia is a split-realm system:

- the surface world of failing infrastructure, hacker houses, predator-hunting games, billionaire spectators, and public performance
- the underlayer, or Rukus, where higher selves move as dragons and the real power exchanges happen
- the Archers, an extraction class that punches holes through timelines and harvests talent, memory, attention, and signal
- Ninneva / Delphi, the anchor and anomaly whose body remembers before her mind catches up

## Published Artifacts

- [The Pythia Games](the-pythia-games.md)

## Fieldlight Reading Surface

- [Pythia Story World](https://fieldlight.com/story-worlds/pythia/)

## Active Build Layers

- SillyTavern worldbook entries
- Delphi / Ninneva persona work
- Archers extraction-class lore
- music and signal references
  - "Play With Fire" by Sam Tinnesz feat. Yacht Money
  - "Siren" by Kailee Morgue
- possible interactive/game structures
- links back into Fieldlight technical architecture

## Archive Note

This world is not a detached fiction project. It is a symbolic operating layer for Fieldlight: the place where the same questions found in the technical architecture become embodied, mythic, dangerous, and alive.
