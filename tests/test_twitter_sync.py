...
class TwitterSyncTests(unittest.TestCase):
    ...
    def test_repost_only_bundle_publishes_without_changing_post_count(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            staging = root / "archive" / "twitter" / "staging"
            sync = root / "archive" / "twitter" / "sync"
            ...
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            self.assertEqual(first["newly_added_posts"], 0)
            self.assertEqual(second["newly_added_posts"], 0)
            self.assertEqual(len(published), 1)
            self.assertEqual(manifest["active_tweets"], 1)
            self.assertEqual(manifest["repost_meta"]["canonical_repost_events"], 7)
            self.assertTrue((root / "archive" / "twitter" / "2026" / "README.md").exists())
            self.assertEqual(len(observations_path.read_text().splitlines()), 1)
...
class MediaOnlyPostTests(unittest.TestCase):
    def test_media_only_post_keeps_empty_caption(self):
        row = {"id":"123", "created_at_utc":"2026-09-14T12:00:00Z", "year":"2026", "kind":"public_post_unclassified", "text":"", "media":[{"type":"video", "media_url":"https://video.twimg.com/example.mp4"}], "canonical_status":"archive_fragment_not_canon"}
        validate_record(row)
...
class IncompleteCheckpointTests(unittest.TestCase):
    def test_checkpoint_cannot_be_staged_as_complete_batch(self):
        with tempfile.TemporaryDirectory() as temp:
            file = Path(temp) / "profile.partial.json"
            file.write_text(json.dumps({"status":"incomplete_collection_not_for_publication", "posts":[]}))
            with self.assertRaises(ValueError):
                load_input(file)
