...
```bash
node tools/x_safari_scraper.mjs \
  --cursor <last_archived_post_id> \
  --cutoff <last_archived_UTC_timestamp> \
  --output .twitter-sync/public-profile-batch.json
python3 tools/twitter_sync.py dry-run \
  --input .twitter-sync/public-profile-batch.json
```

Use the newest ID and timestamp reported by `status` above. The scraper
uses the trusted `x.com/SayitSalty` Safari tab, and stops after it
...
Run with an installed Node.js and Python 3. The collector opens a dedicated profile window if none exists and never replaces an unrelated tab. It waits for rendered posts and advances to the last rendered card to trigger profile pagination.
...
## Stalled browser pagination

For a timeline that stalls during a continuous run, add `--step` and repeat the same command after each page has rendered. The collector retains an ignored `.partial.json` checkpoint tied to the account, cursor and cutoff. It emits the final batch only after crossing the saved boundary. An incomplete checkpoint is never a publication input. If an earlier attempt used a different output or interval, choose a fresh output filename.

The September 14 restoration separately checked Posts and Replies; its coverage and relationship-metadata limits are recorded in [the collection review](2026-09-14-review.md).

## Reposts and collection coverage repair

The regular `scrape` command now traverses Posts and Replies independently before collecting the separate Reposts tab. `tools/x_reposts_safari.mjs PRIVATE_OUTPUT.json` can also collect that surface on its own. It records the stopping condition, preserves a private checkpoint, and emits source observations without inventing repost-action IDs or dates. Keep collector output outside public-writing. Repost-only bundles are supported and repeat imports do not duplicate sightings.

See [September 14 reconciliation](2026-09-14-review.md#gap-repair--september-14-2026) and [the readable repost observations](reposts.md).
