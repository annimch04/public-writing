...
def load_input(
    ...
    if suffix == ".js":
        ...
        if isinstance(payload, dict) and str(payload.get("status", "")).startswith("incomplete"):
            raise ValueError("Incomplete collection checkpoints cannot become publication bundles.")
...
def validate_record(record: dict[str, Any]) -> None:
    required = ("id", "created_at_utc", "year", "kind", "canonical_status")
    ...
    if not isinstance(record.get("text"), str):
        raise ValueError("Sanitized record must contain a text string.")
    if not record["text"] and not record.get("media") and not record.get("quoted_status_id"):
        raise ValueError("Empty public post has no text, media, or quoted status.")
...
def publish_review_bundle(
    ...
    existing_observation_ids = {str(item["observation_id"]) for item in existing_observations}
    new_observation_count = len(set(observations_by_id) - existing_observation_ids)
    ...
    if not proposed and not incoming_observations:
        raise ValueError("The approved bundle contains no public posts or repost observations.")
...
def scrape_command(args: argparse.Namespace) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="fieldlight-x-scrape-") as temp:
        ...
        payloads = []
        try:
            for surface in ("posts", "with_replies"):
                subprocess.run(command + ["--surface", surface], check=True, capture_output=True, text=True)
                payloads.append(read_json(scraped))
            repost_file = Path(temp) / "reposts.json"
            subprocess.run([args.node, str(scraper.with_name("x_reposts_safari.mjs")), str(repost_file), args.username], check=True, capture_output=True, text=True)
            payloads.append(read_json(repost_file))
        except FileNotFoundError as exc:
            raise ValueError(f"Node executable not found: {args.node}") from exc
        except subprocess.CalledProcessError as exc:
            detail = (exc.stderr or exc.stdout or "").strip()
            raise ValueError(f"The public-profile scraper did not complete: {detail or 'unknown error'}") from exc
        combined = {
            "schema_version": 2,
            "source": {"adapter": "trusted_safari_all_public_surfaces", "surfaces": [p["source"] for p in payloads]},
            "posts": [row for payload in payloads for row in payload["posts"]],
            "repost_observations": [row for payload in payloads for row in payload["repost_observations"]],
        }
        scraped.write_text(json.dumps(combined), encoding="utf-8")
